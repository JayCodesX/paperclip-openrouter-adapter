...
async function ensureCommandResolvable(
  ...
  if (command.includes("/") || command.includes("\\")) {
    try { await fs.access(command, fsConstants.X_OK); return; } catch {
  ...
  for (const dir of pathDirs) {
    try { await fs.access(path.join(dir, command), fsConstants.X_OK); return; } catch { /* next */ }
  }
...
function resolveApiKey(config: Record<string, unknown>): string {
  ...
  if (Array.isArray(config.apiKeys)) {
    const first = config.apiKeys.find((k): k is string => typeof k === "string" && k.trim().length > 0);
...
function parseOragerProbeResult(stdout: string): { resultText: string } {
  let resultText = "";
...
export async function testEnvironment(
  ...
  const canRunProbe =
    ...
    checks.every(
      (c) =>
        c.code !== "orager_command_unresolvable" &&
  ...
  if (canRunProbe) {
    ...
    if (dangerouslySkipPermissions) probeArgs.push("--dangerously-skip-permissions");
