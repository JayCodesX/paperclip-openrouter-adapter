...
function parseOragerAssistantEvent(event, ts, rawLine) {
...
function parseOragerUserEvent(event, ts) {
    const message = asRecord(event.message) ?? {};
    const content = Array.isArray(message.content) ? message.content : [];
    const entries = [];
...
function parseOragerResultEvent(event, ts) {
    ...
    const totalCostUsd = typeof event.total_cost_usd === "number" ? event.total_cost_usd : 0;
    const entries = [];
    if (resultText) {
...
function parseHttpResultEvent(parsed, ts) {
 ...
 */
export function parseOpenRouterStdoutLine(line, ts) {
