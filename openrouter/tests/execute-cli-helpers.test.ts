...
// ── checkVisionSupport (5.16) ─────────────────────────────────────────────────
// Unit tests for vision support detection. checkVisionSupport hits /models and
// checks architecture.input_modalities for "image".
...
describe("checkVisionSupport", () => {
  ...
  it("returns false for a text-only model (no image modality)", async () => {
    ...
    const result = await checkVisionSupport("sk-test", "openai/gpt-4o");
    expect(result).toBe(false);
  });

  it("returns true for a model with image modality", async () => {
    fetchSpy.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        data: [{
    ...
    const result = await checkVisionSupport("sk-test", "openai/gpt-4o-vision");
    expect(result).toBe(true);
  });

  it("returns null when model is not found in the list", async () => {
    fetchSpy.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ data: [] }),
    } as Response);

    const result = await checkVisionSupport("sk-test", "openai/unknown-model");
  ...
  it("uses cache on second call — fetch is only called once", async () => {
    fetchSpy.mockResolvedValue({
      ok: true,
      json: async () => ({
        data: [{ id: "openai/gpt-4o", architecture: { input_modalities: ["text"] } }],
      }),
    } as Response);

    await checkVisionSupport("sk-test", "openai/gpt-4o");
    await checkVisionSupport("sk-test", "openai/gpt-4o");
